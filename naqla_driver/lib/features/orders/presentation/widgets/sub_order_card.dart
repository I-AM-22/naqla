  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: REdgeInsets.symmetric(
          vertical: 8, horizontal: UIConstants.screenPadding16),
      child: Container(
                      ...
                      child: RichText(
                        text: TextSpan(
                            ...
                            children: [
                              ...
                              TextSpan(
                                  text:
                                      '${S.of(context).cost}${formatter.format(subOrderModel.cost)} ${S.of(context).syp}\n'),
                              if ((subOrderModel.order?.porters ?? 0) > 0) ...{
                                TextSpan(
                                    text:
                                        '${S.of(context).the_number_of_floors}: ${(subOrderModel.order?.porters ?? 1) - 1}\n'),
                              },
                              TextSpan(
                                  text:
                                      '${S.of(context).order_status}: ${subOrderModel.status?.name}\n'),
                              TextSpan(
                                  text: CoreHelperFunctions.formatOrderTime(
                                      context, subOrderModel.status!,
                                      pickedUpAt: subOrderModel.pickedUpAt,
                                      driverAssignedAt:
                                          subOrderModel.driverAssignedAt,
                        ...
                        SizedBox(
                            ...
                            child: Center(
                                child: AppImage.network(
                                    subOrderModel.photos[0].mobileUrl))),
